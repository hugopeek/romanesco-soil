...
//        $forestbrain = $modx->getService('forestbrain','ForestBrain',$corePath . 'model/forestbrain/', array('core_path' => $corePath));
    ...
    public function renderOutput($rows, $scriptProperties) {
        ...
        if (!empty($o) && !empty($wrapperTpl)) {
            ...
            if ($template[$wrapperTpl]) {
                // CUSTOM
                // Make addfield properties available in wrapper chunk too
                if (!empty($addfields)) {
                    foreach ($addfields as $addfield) {
                        $addfield = explode(':', $addfield);
                        $addname = $addfield[0];
                        $adddefault = isset($addfield[1]) ? $addfield[1] : '';
                        $properties[$addname] = $adddefault;
                    }
                }
                $chunk = $modx->newObject('modChunk');
                $chunk->setCacheable(false);
    ...
    public function prepareGrid($properties, &$controller, &$tv, $columns = array()) {
        ...
        if (is_array($columns) && count($columns) > 0) {
            foreach ($columns as $key => $column) {
                ...
                // Allow columns to be hidden too, based on restrictive condition
                if (isset($column['restrictive_condition'])) {
                    $rc = $this->renderChunk($column['restrictive_condition'], $properties, false, false);
                    if (!empty($rc)) {
                        continue;
                    }
                }
                ...
                foreach ($columnrenderoptions as $column => $options) {
                    ...
                    foreach ($options as $option) {
                        if ($option['_renderer'] == 'this.renderChunk') {
                            ...
                            $properties['_media_source_id'] = $this->config['media_source_id'] ?? null;
                    ...
                    foreach ($fsources as $source) {
                        if (isset($source['context']) && isset($source['sourceid'])) {
                            ...
                            if (!is_numeric($sourceid)) {
                                $uid = uniqid();
                                $chunk = $this->modx->newObject('modChunk', ['name' => "{tmp}-{$uid}"]);
                                $chunk->setCacheable(false);

                                $sourceid = $chunk->process(['content' => $sourceid], '[[+content]]');
                            }

                            // Make sure it is numeric
                            if (!is_numeric($sourceid)) {
                                $this->modx->log(modX::LOG_LEVEL_ERROR, 'Non-numeric media source: ' . $sourceid);
                                $sources[$source['context']] = '';
                            } else {
                                $sources[$source['context']] = $sourceid;
                            }
                        }
                    }
                }
    ...
    function createForm(&$tabs, &$record, &$allfields, &$categories, $scriptProperties) {
        ...
        foreach ($tabs as $tabid => $tab) {
            ...
            //$this->modx->log(modX::LOG_LEVEL_ERROR, print_r($record,1));

            // Allow tabs to be hidden too, based on restrictive condition
            if (isset($tab['restrictive_condition'])) {
                $props = $record;
                $rc = $this->renderChunk($tab['restrictive_condition'], $props, false, false);
                if (!empty($rc)) {
                    continue;
                }
            }
                    ...
                    */
                    ...
                    if (!empty($useDefaultIfEmpty)) {
                        ...
                        if ($tv->get('value') == null) {
                            ...
                            // Process placeholders in default text
                            if (str_contains($v, '[[')) {
                                $v = $this->renderChunk($v, [], false, false);
                            }
